...
public class Editor {
	 ...
	 * @return
	 */
	private boolean isEdge(BufferedImage img, int direction, int pixelA,
			int pixelB, int pixelC) {
		switch (direction) {
		case TOP:
	...
	private BufferedImage cutImage() {
		return img.getSubimage(bounds.getX(), bounds.getY(), bounds.getWidth(),
				bounds.getHeight());
	}

	private BufferedImage scaleThumbnailator(BufferedImage original, int width,
			int height) {
		...
		if (original != null) {
			...
			scaledImage = new FixedSizeThumbnailMaker(width, height, false,
					true).resizer(resizer).make(original);
		}
		return scaledImage;
	}
